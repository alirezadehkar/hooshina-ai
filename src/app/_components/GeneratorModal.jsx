...
export function OpenGeneratorModal({ isOpen, onClose, type = 'text', selectedBlock = null, options = {} }) {
    ...
    const [dismissableModal, setDismissableModal] = useState(true);
    ...
    const handleSpeechVoiceChange = (event) => {
        setSpeechVoice(event?.target?.value || event);
    };

    return (
        <ThemeProvider theme={CustomTheme}>
            <SnackbarProvider>
                <Modal open={isOpen} aria-labelledby="hai-generator-modal" sx={{ zIndex: 130000, overflow: 'hidden', overflowY: 'auto' }}>
                    <Box sx={{
                        width: {
                            xs: '100%', 
                            sm: '550px', 
                            ...
                            <Suspense fallback={
                                ...
                                {showCommentReply || showReviewsSummary ? (
                                    <CommentReplyDisplay
                                        ...
                                        setDismissableModal={setDismissableModal}
                                    />
                                ) : (
                                    <DataDisplay
                                        data={data}
                                        ...
                                        handleSpeechVoiceChange={handleSpeechVoiceChange}
                                        isOpen={isOpen}
                                        onClose={onClose}
                                        selectedBlock={selectedBlock}
                                        options={options}

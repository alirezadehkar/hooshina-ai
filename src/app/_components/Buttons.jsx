...
import { useMutationObserver, buttonActionTypes, HooshinaIcon } from '../Helpers';
...
export const ButtonsInitialize = () => {
    ...
    const registerHooshinaImageReplacementButton = createHigherOrderComponent((BlockEdit) => {
        return (props) => {
            const supportedBlocks = ['core/image', 'core/audio'];

            if (!supportedBlocks.includes(props.name) || !props.isSelected) {
                return <BlockEdit {...props} />;
            }
                ...
                <Fragment>
                    ...
                    <BlockControls>
                        <ToolbarGroup>
                            <ToolbarButton
                                icon={<ToolbarButtonIcon />}
                                label={isImageBlock ? hai_data.texts.image_replace_button : hai_data.texts.audio_replace_button}
                                onClick={(e) => handleOpen(e, (isImageBlock ? 'image' : 'text-to-speech'))}
                            />
                        </ToolbarGroup>
                    </BlockControls>
                </Fragment>
